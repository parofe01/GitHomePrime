...
function initShaders() {
  ...
  // Obtener la ubicación del atributo de normales del shader
  program.vertexNormalAttribute = gl.getAttribLocation(program, "VertexNormal");
  // Obtener la ubicación de la matriz de normales (uniform)
  program.normalMatrixIndex = gl.getUniformLocation(program, "normalMatrix");
  // Habilitar el atributo de normales
  gl.enableVertexAttribArray(program.vertexNormalAttribute);

  //Obtención de las referencias a las variables del shader que contendrán el material
  program.KaIndex    = gl.getUniformLocation(program, "Material.Ka");
  program.KdIndex    = gl.getUniformLocation(program, "Material.Kd");
  program.KsIndex    = gl.getUniformLocation(program, "Material.Ks");
  program.alphaIndex = gl.getUniformLocation(program, "Material.alpha");

  //Obtención de las referencias a las variables del shader que contendrán los valores de la fuente de luz
...
//Funciones para el cálculo y la inicialización de la matriz de la normal en el shader a
...
//Recibe un material como parámetro e inicializa las variables del shader correspondientes
...
//Inicializa las variables del shader correspondientes
...
//Nueva función de dibujo que incluye dos atributos: posición y normal
function drawSolid(model) {
  // Enlazar el buffer de vértices (posiciones + normales)
  gl.bindBuffer(gl.ARRAY_BUFFER, model.idBufferVertices);

  // Atributo de posición (3 floats = 12 bytes)
  gl.vertexAttribPointer(
    program.vertexPositionAttribute,
    3, gl.FLOAT, false,
    2 * 3 * 4, // stride: 6 floats * 4 bytes
    0          // offset: comienza en 0
  );

  // Atributo de normal (3 floats = 12 bytes)
  gl.vertexAttribPointer(
    program.vertexNormalAttribute,
    3, gl.FLOAT, false,
    2 * 3 * 4,  // mismo stride
    3 * 4       // offset: después de los 3 floats de posición
  );

  // Enlazar el buffer de índices
  gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, model.idBufferIndices);

  // Dibujar el modelo como triángulos
...
function updateCameraUniforms(modelMatrix) {
    ...
    gl.uniform3fv(program.PositionIndex, eye);
}
