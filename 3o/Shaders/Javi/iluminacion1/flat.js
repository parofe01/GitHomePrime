// Es el MISMO JS que cascos_phong.js (mismo modelo y cámara).
// La diferencia del sombreado está en los shaders del HTML.
...
function drawSolid(model) {
    ...
    gl.vertexAttribPointer(program.vertexPositionAttribute, 3, gl.FLOAT, false, 6 * 4, 0);
    gl.vertexAttribPointer(program.vertexNormalAttribute,   3, gl.FLOAT, false, 6 * 4, 3 * 4);
...
function drawCascos(view) {
    drawCubePart(view, [-3, 0.0, 0.0], [0,0,0], [2, 3, 3]);
    drawCubePart(view, [ 3, 0.0, 0.0], [0,0,0], [2, 3, 3]);

    drawCubePart(view, [ 2.5, 2.5, 0.0], [0,0,0], [1, 2, 1]);
    drawCubePart(view, [-2.5, 2.5, 0.0], [0,0,0], [1, 2, 1]);
